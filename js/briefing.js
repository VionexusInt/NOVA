...
async function notificarWindows(titulo, mensaje) {
  try {
    ...
    // El agente puede no estar corriendo o win10toast no instalado — no es crítico
  }
}
...
async function getNews() {
  const feeds = [
    ...
    'https://www.lasprovincias.es/rss/2.0/?section=espana'
  ];
  for (const url of feeds) {
    try {
      ...
      if (d.status === 'ok' && d.items?.length > 0) {
        return d.items.slice(0,3).map(i => i.title).join(' · ');
      }
    } catch(e) { continue; }
  }
  return null;
...
export async function briefingAutomatico() {
  ...
  const resumenNotificacion = urgentes.length > 0
    ? `${urgentes.length} tarea${urgentes.length > 1 ? 's' : ''} urgente${urgentes.length > 1 ? 's' : ''} pendiente${urgentes.length > 1 ? 's' : ''}.`
    : 'Briefing completado sin urgencias.';
  notificarWindows('NOVA — Briefing', resumenNotificacion);
}
